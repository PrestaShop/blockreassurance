...
class AdminBlockListingController extends ModuleAdminController
{
    ...
    /**
     * ajaxProcessChangeBlockStatus
     *
     * @return bool
     */
    public function ajaxProcessChangeBlockStatus()
    {
        $iPsreassuranceId = (int) Tools::getValue('id');
        $iStatus = (int) Tools::getValue('status');

        $aUpdatedDatas = array(
            'status' => $iStatus
        );
        $sWhere = 'id_psreassurance = '.$iPsreassuranceId;

        $bStatusChanged = Db::getInstance()->update('psreassurance', $aUpdatedDatas, $sWhere);
